...
describe ControllerBase do
  ...
  before(:all) do
    class TestCatsController < ControllerBase
      def index
        @cats = ["GIZMO"]
      end
    end
  end
  after(:all) { Object.send(:remove_const, "TestCatsController") }

  let(:req) { WEBrick::HTTPRequest.new(Logger: nil) }
  let(:res) { WEBrick::HTTPResponse.new(HTTPVersion: '1.0') }
  let(:cats_controller) { TestCatsController.new(req, res) }
  ...
  describe "#render" do
    ...
    describe "#already_built_response?" do
      let(:cats_controller2) { TestCatsController.new(req, res) }
